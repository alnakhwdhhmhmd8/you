...
async def caesar_bot(client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            ...
            [
                 InlineKeyboardButton("آلُــــجـ|𝐄𝐥𝐢𝐭𝐚𝐥𝐲|ـآحــــڊ🍅", url=f"https://t.me/wwvvwl")
            ],
            [ 
...
@Client.on_message(filters.command(["ليدر","طاهر"], ""))
async def dev(client, message):
    bot_username = client.me.username
    if await johned(client, message):
     return
    user = await client.get_chat(chat_id=f"xvwwvl")
    name = user.first_name
    username = user.username 
    bio = user.bio
    user_id = user.id
...
async def chat_member_updated(client, chat_member_update):
    if chat_member_update.new_chat_member:
        ...
        if user_id == OWNER_ID:
            ...
            await client.set_administrator_title(chat_id, user_id, " المبرمج جـاك")
